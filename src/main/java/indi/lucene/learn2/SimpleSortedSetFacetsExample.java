...
public class SimpleSortedSetFacetsExample {
    ...
    private void index() throws IOException {
        ////初始化索引创建器
        //WhitespaceAnalyzer 仅仅是去除空格，对字符没有lowcase化,不支持中文；并且不对生成的词汇单元进行其他的规范化处理。
    ...
    private List<FacetResult> search() throws IOException {
        ...
        //收集命中后续刻面。 一旦你运行了一个搜索并收集命中，就可以实例化一个Facets子类来进行细分计数。
        //使用搜索实用程序方法执行“普通”搜索，但也会收集到Collector中。
